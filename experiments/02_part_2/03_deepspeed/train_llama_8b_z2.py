...
class ModifiedModel(torch.nn.Module):
    def __init__(self, base_model):
        ...
        self.classifier = torch.nn.Linear(base_model.config.hidden_size, 2)  # 二分类
    ...
    def forward(self, input_ids, attention_mask=None):
        ...
        hidden_states = outputs.hidden_states[-1]  # 获取最后一层的 hidden_states
        logits = self.classifier(hidden_states[:, 0, :])  # 只使用 [CLS] token 的 hidden state 进行分类
        return logits
...
class CustomTrainer(Trainer):
    def compute_loss(self, model, inputs, return_outputs=False):
        ...
        # 确保 logits 是浮点类型，labels 是长整型
        logits = logits.float()
        labels = labels.long()

        # 检查标签的范围
        if torch.any(labels >= logits.size(-1)):
            raise ValueError("标签值超出范围")
...
def get_model():
    ...
    return model, modified_model, tokenizer

model, modified_model, tokenizer = get_model()
...
def preprocess_function(examples):  
    ...
    # 随机生成二分类标签
    # labels = [random.randint(0, 1) for _ in range(len(contexts))]
